...
| `RATE_LIMIT_MAX` | `8` | Paid checks per IP per minute. A real check session is about 6 requests, so this leaves room for a presenter running the demo twice. Loosening it does not raise daily exposure, because the hourly ceiling below is the binding tier. |
